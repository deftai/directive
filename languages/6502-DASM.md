...
- ! Keep branch-distance limits in mind: relative branches are signed 8-bit offsets (âˆ’128..+127 from next PC).
...
- âŠ— Assume all indirect control transfers are interchangeable; account for instruction-specific behavior and timing.
...
- â‰‰ Use decimal mode (`D`) unless your target/runtime explicitly requires and supports that behavior in-context.
...
- â‰‰ Micro-optimize everywhere; optimize measured hotspots first.
...
- âŠ— Use scratch RAM assumptions that break under nested interrupts or reused dispatch paths.
