...
- ! Consistent brace style (K&R or Allman â€” pick one, enforce via clang-format)
...
- âŠ— Pollute global namespace with short, generic names
...
- ! Use `enum` for related constants; âŠ— `#define` chains for enumerable values
- ~ Use `const` aggressively for read-only data and pointer targets
- âŠ— Rely on implicit `int` (removed in C99+)
- âŠ— Assume `char` is signed or unsigned â€” use `unsigned char` or `uint8_t` for byte data
...
- âŠ— Use `realloc` on `NULL` without checking the return value
- âŠ— Cast the return value of `malloc` (unnecessary in C, hides missing `#include`)
...
### Strings & Buffers
- ! Always bounds-check string operations; âŠ— `strcpy`, `strcat`, `sprintf`, `gets`
...
- âŠ— Silently ignore return values of I/O, allocation, or system calls
...
- âŠ— Define macros that rely on implicit variable names from the calling context
- âŠ— Use `#define` to redefine standard library functions or keywords
...
- âŠ— Use `volatile` as a synchronization mechanism (it is not one)
...
### Undefined Behavior
- âŠ— Rely on undefined behavior (signed overflow, null deref, out-of-bounds, etc.)
...
- âŠ— Use `system()` with user-supplied input
...
- â‰‰ **Deep nesting**: Use functions or `goto cleanup`
- â‰‰ **Tagless typedef struct**: Include struct tag for forward declarations
- â‰‰ **Mixing signed/unsigned**: Explicit casts + `-Wconversion`
- â‰‰ **Global mutable state**: Pass state via struct pointers
...
- ! Bounds-checked string/buffer operations; âŠ— `strcpy`/`sprintf`/`gets`
- ! Follow SEI CERT C for security-sensitive code
- âŠ— Undefined behavior (signed overflow, null deref, out-of-bounds)
