 ...
 * (`scripts/monitor_pr.py`) and the ported TS CLI with a fake `gh`
...
export const PARITY_SCENARIOS: readonly ParityScenario[] = [
  ...
  {
    name: "clean-first-poll-human",
    argv: ["1363", "--repo", "deftai/directive", "--cap-minutes", "60"],
    responses: {
      "head-sha": { returncode: 0, stdout: `${HEAD_SHA}\n` },
      "comments-jq": { returncode: 0, stdout: cleanJqBody() },
    },
  ...
  {
    name: "transient-error-cap-zero-json",
    argv: ["1363", "--repo", "deftai/directive", "--json", "--cap-minutes", "0"],
    responses: {
  ...
  {
    name: "blocked-cap-zero-human",
    argv: ["1363", "--repo", "deftai/directive", "--cap-minutes", "0"],
    responses: {
      "head-sha": { returncode: 0, stdout: `${HEAD_SHA}\n` },
...
function installFakeGh(): { binDir: string; cleanup: () => void } {
  const binDir = mkdtempSync(join(tmpdir(), "deft-pr-monitor-fake-gh-"));
...
function runScenario(
  ...
  try {
    ...
    const py = runCapture(
      "uv",
      ["run", "python", join(deftRoot, "scripts", "monitor_pr.py"), ...scenario.argv],
      deftRoot,
      env,
    );
    const ts = runCapture(
      "node",
      [join(deftRoot, "packages", "cli", "dist", "pr-monitor.js"), ...scenario.argv],
      deftRoot,
      env,
    );
    return {
...
export function renderReport(result: ParityResult): string {
  if (result.ok) {
    return `monitor_pr parity: CLEAN -- Python and TS agree on ${result.scenarios.length} scenario(s).`;
  }
  const lines = ["monitor_pr parity: DIVERGENCE"];
...
if (process.argv[1] !== undefined && fileURLToPath(import.meta.url) === process.argv[1]) {
  try {
    ...
    process.stderr.write(`monitor_pr parity: harness error -- ${msg}\n`);
    process.exit(2);
  }
}
