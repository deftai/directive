...
describe("named-cause gate failures (#3282)", () => {
  ...
  it("names the basename remedy when the filename contains an apostrophe (#4844)", () => {
    const msg = formatNamedCauseFailure({
      gateId: "vbrief:validate",
      exitCode: 1,
      stdout:
        "FAIL: xbrief/completed/2026-01-01-owner's-plan.xbrief.json: filename '2026-01-01-owner's-plan.xbrief.json' does not match convention YYYY-MM-DD-descriptive-slug.xbrief.json (D7)\n",
    });
    expect(msg.remedy).toContain("already landed");
    expect(msg.remedy).not.toContain("schema");
    expect(msg.remedy).not.toBe("Fix xBRIEF/vBRIEF schema errors reported by the gate");
