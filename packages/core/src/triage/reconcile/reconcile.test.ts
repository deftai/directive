...
describe("reconcile", () => {
  it("restores missing accepts", () => {
    ...
    scopeVbrief(join(root, "xbrief", "proposed"), "a", 2);
    scopeVbrief(join(root, "xbrief", "proposed"), "b", 3);
    const result = reconcile(root, { repo: "deftai/directive" });
    expect(result.exitCode).toBe(0);
    expect(result.restored).toBe(2);
    const log = readFileSync(join(root, "xbrief", ".triage-cache", "candidates.jsonl"), "utf8");
  ...
  it("finds reconcilable items read-only", () => {
    const root = mkdtempSync(join(tmpdir(), "reconcile-"));
    scopeVbrief(join(root, "xbrief", "pending"), "c", 4);
  ...
  it("countReconcilable honors restrictTo", () => {
    ...
    scopeVbrief(join(root, "xbrief", "pending"), "c", 4);
    scopeVbrief(join(root, "xbrief", "pending"), "d", 5);
