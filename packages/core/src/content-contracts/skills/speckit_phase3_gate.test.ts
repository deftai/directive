...
/** Port of tests/content/test_speckit_phase3_gate.py (#1838 #432 #2050) */
...
describe("test_speckit_phase3_gate", () => {
  describe("TestSpeckitPhase3TransitionGate", () => {
    it("post_phase_3_is_numbered_transition_gate", () => {
      ...
      expect(speckitText).toContain("Export for Review");
    });
    ...
    it("transition_criterion_references_export_succeeded", () => {
  ...
  describe("TestSetupSkillPhase3RenderBoundary", () => {
    ...
    it("setup_invokes_task_project_export_spec_at_boundary", () => {
      expect(setupText).toContain("task project:export-spec");
    });

    it("setup_prompts_for_prd_and_spec", () => {
      expect(
        setupText.includes("stakeholder-facing spec export") ||
          (setupText.includes("SPECIFICATION.md") && setupText.includes("PRD.md")),
      ).toBe(true);
    });
    ...
    it("speckit_phase_4_gate_wiring", () => {
      ...
      expect(setupText).toContain("export succeeded");
    });
  });
});
