...
function readSwarmSurface(): string {
  const order = [
    ...
    "skills/deft-directive-swarm/references/core-phase-0.md",
    "skills/deft-directive-swarm/references/core-phase-1-2.md",
    "skills/deft-directive-swarm/references/core-phase-3.md",
    "skills/deft-directive-swarm/references/host-warp.md",
...
describe("test_allocation_context_skills.py", () => {
  ...
  for (const token of [
    ...
    it(`test_swarm_phase3_population_token_present ${token}`, () => {
      expect(swarmStep0(readSwarmSurface())).toContain(token);
    });
  }
  it("test_swarm_phase3_population_is_must_step", () => {
    const block = swarmStep0(readSwarmSurface());
  ...
  it("test_swarm_phase3_population_has_absent_section_prohibition", () => {
    let found = false;
    for (const line of swarmStep0(readSwarmSurface()).split("\n")) {
      if (
  ...
  for (const token of [
    ...
    it(`test_swarm_phase3_step1a_sandbox_auth_token_present ${token}`, () => {
      expect(swarmStep1a(readSwarmSurface())).toContain(token);
    });
  }
  it("test_swarm_phase3_step1a_is_must_step", () => {
    expect(swarmStep1a(readSwarmSurface())).toContain(
