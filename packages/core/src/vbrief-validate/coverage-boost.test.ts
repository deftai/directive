...
describe("vbrief-validate coverage boost", () => {
  ...
  it("exercises validate CLI success and failure paths", () => {
    ...
    writeFileSync(
      join(vbrief, "active", "2026-01-01-bad-status.xbrief.json"),
      JSON.stringify({
        xBRIEFInfo: { version: "0.8" },
  ...
  it("exercises epic link backward and forward branches", () => {
    const vbrief = "/tmp/vb";
    const parent = join(vbrief, "proposed/parent.xbrief.json");
    const child = join(vbrief, "pending/child.xbrief.json");
    const all = new Map<string, Record<string, unknown>>([
      [
        parent,
        {
          plan: {
            references: [{ type: "x-vbrief/plan", uri: "pending/child.xbrief.json" }],
          },
        },
      ],
      [
    ...
    const display = new Map([
      [parent, "xbrief/proposed/parent.xbrief.json"],
      [child, "xbrief/pending/child.xbrief.json"],
    ]);
  ...
  it("exercises validateAll load errors and discover", () => {
    ...
    const vbrief = join(root, "xbrief");
    mkdirSync(join(vbrief, "active"), { recursive: true });
    writeFileSync(join(vbrief, "active", "2026-01-01-not-json.xbrief.json"), "{bad", "utf8");
  ...
  it("exercises plan hook and origin edge branches", () => {
    ...
    expect(validateOriginProvenance("xbrief/proposed/x.xbrief.json", {}, "xbrief")).toEqual([]);
    ...
    expect(validateOriginProvenance("xbrief/pending/l.xbrief.json", legacyExt, "xbrief")).toEqual(
      [],
    );
    const legacySlash = {
      plan: { status: "pending", references: [{ type: "github-issue/legacy", uri: "x" }] },
    };
    expect(validateOriginProvenance("xbrief/pending/s.xbrief.json", legacySlash, "xbrief")).toEqual(
      [],
    );
  });
  ...
  it("normalizes trailing slashes in display paths (ReDoS-safe linear scan)", () => {
    ...
    mkdirSync(join(vbrief, "active"), { recursive: true });
    writeFileSync(
      join(vbrief, "active", "2026-01-01-slug.xbrief.json"),
      JSON.stringify({
        xBRIEFInfo: { version: "0.8" },
