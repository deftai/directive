...
`CANONICAL_OWNER_PATTERN` is derived from the provider list and lives beside the provider table in `session/host-session-owner.ts`, so a provider added to the identity surface is added to the lifecycle-rewrite surface and to grant-time child validation in the same edit. Drift between them is what left a host able to resolve an owner it could never bind a claim under (#3873), and what let `occupancy:grant` record a child id no session could present (#3954).
