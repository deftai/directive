...
export const RETIRED_UNMANAGED_HEADER_SECTIONS = ["## Status", "## Known Issues", "Next:"] as const;
...
export function containsRetiredUnmanagedHeaderPatterns(text: string): boolean {
  ...
  return RETIRED_UNMANAGED_HEADER_SECTIONS.some((pattern) => normalized.includes(pattern));
}
