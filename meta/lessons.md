...
**3. The lesson's prior wording was insufficient -- backtick-wrapping does NOT escape the parser; never cite real issue numbers after a closing-keyword token in any form**

The PR that introduced this lesson section auto-closed the umbrella issue a SECOND time on squash merge -- despite the PR body wrapping the offending `` `<keyword> #<NNN>` `` token in inline code (single backticks) inside a paragraph that was DESCRIBING the prior incident. (Where `<keyword>` is one of the closing-keyword verbs and `<NNN>` is a real issue number, both omitted here on purpose to keep this lesson self-protecting.) GitHub's closing-keyword parser treats inline code spans as plain text for keyword detection. Backtick-wrapping renders the token as code visually, but does NOT hide it from the parser. The umbrella was reopened a second time with explanatory comment per Rule 2 (post-merge verification), confirming the rule's bidirectional value but also revealing that the rule's prior phrasing was too soft.

**Stronger rule:** A PR body MUST NOT contain any closing-keyword token followed by a literal issue number, in any markdown context -- not in prose, not in inline code spans (single or double backticks), not in block quotes, not in negations, not in citations of prior incidents. The parser is substring-based and ignores markdown formatting except (per GitHub docs) genuine fenced code blocks (triple backticks). Even fenced code blocks SHOULD NOT be relied on as a workaround; they are an escape hatch for code samples, not a license to embed live triggers. The same rule applies to commit messages and PR titles -- the squash commit subject is a closing-keyword surface too.

**Defensive convention for documents and PR bodies that need to discuss the closing-keyword behavior (this lesson, `scm/github.md`, PR templates, RFCs):**

- Use placeholder issue numbers like `#N` or `#NNN` after any closing-keyword token (e.g. `Closes #N`, `Fixes #NNN`). The parser needs a real digit string to match -- a literal `N` after the hash is inert.
- When citing real prior incidents, refer to the issue and PR numbers SEPARATELY from the verb. Use the issue number on its own (`#NNN`) and describe the action with non-trigger verbs (e.g. "auto-closed", "shut", "marked done"). Do NOT place any of the trigger verbs (`close`, `closes`, `closed`, `fix`, `fixes`, `fixed`, `resolve`, `resolves`, `resolved`) immediately before a real `#<digit>` reference.
- For lesson and convention documents that MUST discuss the literal token shape: name it as `<keyword> #<NNN>` with angle-brackets, not as a literal example.
- Lessons.md itself is not parsed -- but treat it as if it were, because its prose is regularly copied into PR bodies, commit messages, and review comments where parsing DOES happen.

**Self-test for this section:** after any edit, run a case-insensitive regex check on the entire "GitHub Closing-Keyword False-Positive" section: `(close[ds]?|fix(e[ds])?|resolve[ds]?)\s*#\d+`. The match count MUST be zero. Any future edit that introduces a match MUST be reverted before commit.

**Cross-reference (extension):** the original Cross-reference list above; the second-bite incident referenced in this rule's first paragraph; and the self-test regex above co-located in this section as a tripwire.
