...
## Runner Pool Reliability (2026-04, post-v0.20.0)

**1. Third-party self-hosted runner pools (Blacksmith, Ubicloud, etc.) can starve under burst load -- release-critical jobs MUST fall back to GitHub-hosted runners**

Third-party runner services like Blacksmith advertise faster per-minute execution and lower cost than GitHub-hosted runners, but their capacity pools are smaller. During the v0.20.1 release cut, Blacksmith's `blacksmith-4vcpu-ubuntu-2404` pool starved the `darwin/arm64` matrix cell of `.github/workflows/release.yml` for 54 minutes while every other cell in the same matrix picked up a runner in <60 seconds. The operator judged the queue-time SLA unsalvageable and cancelled the run; the darwin/arm64 job happened to complete 9 seconds before the cancel propagated to the downstream `universal-macos`, `smoke-test`, and `release` jobs, which were therefore never given a chance to run. The v0.20.1 tag became a zombie (no binaries published, no release attached), and the work had to be retagged as v0.20.2 after switching the `build` and `release` jobs to `ubuntu-latest`. This was the second Blacksmith-side incident in the same workflow this release cycle -- the prior one was a macOS networking issue (ECONNREFUSED on `actions/download-artifact@v4`) that forced `universal-macos` to `macos-latest` as a workaround. Pattern: any job whose failure or delay blocks a user-visible release (build matrix, release-publish, tag-triggered workflows) MUST use GitHub-hosted runners (`ubuntu-latest`, `macos-latest`, `windows-latest`) by default. Non-release CI jobs (lint, test, content checks) MAY use third-party runners for speed/cost, with the understanding that a transient pool starvation is merely annoying, not release-blocking. When a job is switched from a third-party pool to GitHub-hosted due to an incident, add an inline comment on the `runs-on:` line documenting the reason so the rationale survives future maintainer audits and prevents silent reversion.

Corollary: **release workflows MUST include a monitoring contract that escalates on queue-time starvation, not just on job failure**. A workflow that is structurally sound but has an unassigned runner for 30+ minutes is functionally equivalent to a hung workflow. The release-cut playbook MUST include a ~15-minute queue-time gate after tag push: if any job remains in `status=queued` without a `runner_name` assignment past that threshold, escalate to the operator (cancel + diagnose, or accept indefinite wait). Silently waiting for a runner that will never come is indistinguishable from success from the tag-pusher's perspective until hours later. (v0.20.1 incident, 2026-04-23)
