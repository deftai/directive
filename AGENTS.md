...
- ! Dispatcher-level lifecycle hygiene (capability-tiered, #3158 / #954): default — workers MUST be all-or-nothing on their dispatch envelope; mid-scope user-approval gates require two separate dispatches (Scope A → worker reports back → user approves → Scope B) when the host treats a paused worker as terminal (`agent_id` unreachable after tool-loop exit). When the host **retains** a live, addressable child (continue-by-agent-id / resume-by-name / steerable mid-flight), a single dispatch MAY include a mid-scope gate and the parent re-messages the same child — do not force a second full dispatch solely for the gate. Capability-gate via platform descriptor / host adapter; hosts without retain keep split-dispatch. Retention is orchestration only — not mid-run constitution self-edit (#3164). Topology bounds: temporary deferral to #3155. See `content/templates/agent-prompt-preamble.md` §10; always-on pin: `## Mid-scope gate capability tier (#3158 / #954)`.
