...
4. Strategy output shape violations (run `python -m scripts.validate_strategy_output --project-root <path>` or the installed equivalent under `.deft/core/scripts/`):
...
> "This project was generated with pre-v0.20 or non-conformant strategy output. Run the deterministic validator and follow its remediation: `python scripts/validate_strategy_output.py --project-root .` (or `.deft/core/scripts/validate_strategy_output.py` after install). Then `task migrate:vbrief` / `task project:render` / strategy re-run as indicated."
