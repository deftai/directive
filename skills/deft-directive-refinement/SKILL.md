...
What would you like to do with this candidate?
  ...
  3. Defer        -- delegate to `task triage:defer <issue> -- --reason "..." [--resume-on "<expr>"]` (non-terminal; the resume condition surfaces the item under `[RESUME]` when it fires)
...
- ! Defer flags (#1123 / D3). `task triage:defer` requires `--reason "..."` (structured rationale on the audit entry) and optionally accepts `--resume-on "<expression>"`. Grammar v1 atomics: `ref:closed:#N` | `ref:merged:#N` | `date:>=YYYY-MM-DD` | `pending-count:>=N` | `pending-count:<=N`. Top-level composition: a single `AND` or `OR`. When the condition fires (evaluated by `task triage:audit --evaluate-resume` or `task triage:refresh-active`), a `resume-eligible` audit row is appended referencing the prior defer; D11's queue surfaces it under `[RESUME]` and D2's summary counts it under `stale-defer (resume condition met)`. The framework does NOT auto-un-defer -- the operator re-triages with current data.
