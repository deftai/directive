...
The three tokens are the blocking, sharpening, and footnote classes `walk all` already consumes in that order. This contract does not add a decision table between blocking and sharpening; adequacy of that choice is critic judgment. A `footnote` cannot carry disposition weight: it is in the census, it is not residual, and it is not in the auto-stamp denominator. Anatomy is required of blocking and sharpening findings; a finding that cannot name evidence, a failure mode, and a disposition consequence is a footnote, not a silent skip of classification.
