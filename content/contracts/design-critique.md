...
- ! Before the first sibling spawn in an N≥3 panel, the parent records three distinct family names. The panel-deposit MAY add `families: <family-1>, <family-2>, <family-3>`; an equivalent set of `sibling-seat` records MAY carry one `family: <name>` each. Either record is dispatch bookkeeping, not Stop 5 evidence.
- ! A three-seat same-family set is not a panel. The parent MUST NOT lean on it as corroboration. Recovery is to re-seat the missing families or halt with the existing halt line; waiting for more same-family agreement does not repair composition.
- ! Cross-family names do not establish independence by themselves. Stop 5 still requires primary-source re-derivation or a cross-family re-check, so composition is not evidence.
