...
- Fingerprint: compose with #3448. Primary fingerprint (heading ids when present, else the primary still-open finding heading/id) plus a material-delta conjunct. Two consecutive retries with that primary unchanged and no material take movement = same-fingerprint halt. Restatement is not progress. ⊗ Invent a clustering predicate. ⊗ Treat a restating successor lean as movement. A critic-side stable-id is cheaper than a new predicate; still-open finding headings/ids already exist in this text.
...
- **Parallel fingerprint.** Parallel siblings cannot coordinate heading tokens; Envelope and ceiling purchases that isolation. Merging sibling maps does not produce a fingerprint comparable across rounds. *Working default:* no comparable fingerprint exists for a parallel round. A parallel round has no early-halt path and runs to the operator-gated cap. Cap halt is the spend bound. Independent critic targeting of an audit marker still clears it; unsoundness is an ordinary classified finding. ⊗ Restrict a sequential-only halt that still never fires. ⊗ Canonicalize sibling headings before computing the fingerprint. ⊗ Cluster untrusted thread text.
