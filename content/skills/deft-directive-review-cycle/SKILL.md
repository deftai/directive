...
- **B.** Parent-retained ownership: parent does **not** mark the work complete. Emit `review_cycle: in_progress:<pr>#parent-retained` (or lease id when a sticky lease exists). When dest spawn is available, parent-retained is closer plus post-merge `scope:complete` **after** a dest worker exists — next action is dest-cwd class A residual or engine wait, not parent-inline poll/fix (#4821). When dest spawn is unavailable (Tier 3), the next concrete action remains explicit dual-source poll/fix (Step 1 + Step 6).
...
! **Cohort through-merge intent is unchanged:** stories still land on master. Envelope selection only assigns **who owns implement vs who owns Greptile/CI/merge** under capacity stall, conf floors, wall-clock budgets, or large multi-gate stories. Happy-path single `drive-to: merge-ready` leaves remain the default on hosts other than Grok through-merge and do not use this partner handoff. **Grok through-merge (#4529 / #4821)** always uses this partner handoff (`stop-at: pr-open` + dest class A residual + Approach 1 wait + parent-retained closer). Implement stays `stop-at: pr-open`.
...
2. ! The sub-agent polls using the mechanism for its primitive: for `spawn_subagent` the **child** owns `pr:watch` / gh poll in its own process (adaptive cadence: ~20-30s first check after push, ~60s second, ~90s thereafter; Greptile typically lands in 3-7 min) — the user-facing parent MUST NOT enter `get_command_or_subagent_output`; a named durable-owner session MAY pull-wait (`host-grok-build.md` Gap D); for `start_agent` the native messaging path; for the Cursor `Task` tool the backgrounded-task completion-notification path; for Claude Code `Agent` the backgrounded-agent completion-notification path; for OpenClaw `sessions_spawn` the host session completion / messaging channel (prefer `task pr:watch` inside the child when available). Front-load the first check to catch fast reviews.
...
4. ! The main conversation pane stays fully interactive during monitoring -- the user (or parent monitor) can continue other work. On Grok Build this is conditional until a measured completion-notify exists: it holds when the parent returns the turn after background spawn, not when the parent pull-waits (`host-grok-build.md` Gap D / #4796).
...
⚠️ **Swarm / hybrid limitation**: Approach 2 is NOT autonomous for swarm agents or manual worktree setups. Yielding ends the turn with no self-wake; the parent monitor (or external scheduler) must detect idle and re-trigger or send a message. For a grok-build **parent**, prefer Approach 1: background `spawn_subagent` then end the parent turn (`host-grok-build.md` Gap D). The **child** owns the poll. The user-facing parent MUST NOT enter `get_command_or_subagent_output`. If this session is the named durable owner (second-session split) it MAY pull-wait; that session is then not the interactive research pane. A grok-build **leaf** blocks on `pr:watch` in-process (#4130); it MUST NOT nested-spawn that poller.
