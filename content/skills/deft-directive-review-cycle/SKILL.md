...
1. ! Spawn **or** retain **exactly one** review-cycle owner for the open PR:
   ...
   ! **Grok through-merge named partner (#4529):** Approach 1 sibling or parent-retained only. Phase 6 monitor squash-merges via `pr:wait-mergeable-and-merge` (or parent-retained). `swarm:finalize-cohort` is leftover after merge. ⊗ Harvest option 2 (Grok `drive-to: merge-ready` continuation) as this closer. ⊗ Use `merge-release` as this closer. Related: #4421.
