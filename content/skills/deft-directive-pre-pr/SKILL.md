...
Structured self-review loop agents run before submitting a PR. Catches inconsistencies, missing enforcement markers, incomplete acceptance criteria, scope creep, and unintended changes before they reach the reviewer. After the RWLDL loop, Phase 6 runs a four-focus specialist pass (A acceptance+tests, B bug hunt, C stealth/secrets/log leaks, D regression+extra scope).
...
## Exit Condition (RWLDL)

! Exit the RWLDL loop when a complete Read-Write-Lint-Diff cycle produces **zero changes** -- no file edits in Write, no lint fixes in Lint, and no scope issues in Diff.

After exiting RWLDL:
- ! Run `task check` one final time to confirm clean state
- ! Run Phase 6 (four-focus) before push -- RWLDL-clean is not push-ready

## Phase 6 -- Four-focus (A/B/C/D)

! After the RWLDL loop exits, run the four-focus specialist pass before push. Canonical prompts, output schema, dual-stop, and host fallback: [`references/four-focus.md`](references/four-focus.md).

- ! Collect `merge-base..HEAD` diff + changed files + the active xBRIEF (if any)
- ! Launch Pass A (acceptance+tests), B (bug hunt), C (stealth/secrets/log leaks), D (regression+extra scope) as four read-only specialists in parallel when the host supports it; sequential fallback only when subagents/workflow are unavailable
- ! On Grok, prefer `.grok/workflows/pre-pr-four-focus.rhai` (`/pre-pr-four-focus`)
- ! Synthesize findings; fix in-scope P0/P1; re-run `task check` and four-focus. Dual-stop: success = zero P0/P1; failure = 2 re-runs then halt with an operator-visible report
- ⊗ Push because RWLDL / `task check` was green if four-focus did not run
- ⊗ One agent "doing all four passes in one thought" when subagents or the Grok workflow are available
- ⊗ Expand PR scope to absorb Pass D extra-scope findings -- file them instead

After Phase 6 is clean (or the operator records an explicit waiver in the PR body):
