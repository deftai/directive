...
- ! **Run `task pr:check-closing-keywords` before opening the PR (default mode `both` = FP #737 + intent #3015).** Offline: `--body-file` / `--commits-file`. When full DoD close is intentional, either add a body trailer `deft-close-intent: full` or pass `--allow-close <N,M>`. FP-only: `--mode fp`. Known-safe quoted tokens: `--allow-known-false-positives`.
