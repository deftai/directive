...
- ! **Merge chokepoint (once after the loop, before push/PR):** run full `deft check` first, then the tree-correct task form (`task deft:check` on include-only consumers; `task check` in framework source) once before push/PR. Do not add a fourth probe (#2893 / #4379). A bare `task check` miss on an include-only consumer is not "gate unavailable". This is a #1704 revision of the prior Phase 3 Lint exit + final confirm (two runs, one inside every iteration). The escape-rate guard below still applies; do not invent a separate surface.
...
After exiting:
- ! Run full `deft check` first, then the tree-correct task form (`task deft:check` on include-only consumers; `task check` in framework source) once — the merge chokepoint. Do not add a fourth probe (#2893 / #4379). Phase 3 used the iteration lane, so this is the first full suite in the loop, not a second run.
