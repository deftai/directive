...
! **Default for story / through-merge dispatch:** `drive-to: merge-ready` — the implementation leaf owns implement → pre-pr → push → PR → Greptile review-cycle → CI → merge-ready (and `scope:complete` when the envelope includes that step) as **one** unit (#1880 Gap C / #3032). **Grok through-merge exception (#4529):** this default does **not** apply. Implement MUST be `stop-at: pr-open`. Named partner is Approach 1 sibling or parent-retained review owner. Phase 6 / `swarm:finalize-cohort` is the sole squash-merge and leftover / completed-tracked owner. This MUST is Grok through-merge only — not a global Envelope SLA recut.
...
| Host leaf cannot nest a review-monitor (Cursor Task / Claude Code nested-spawn limits #2797 / #3134) **and** Approach 1 sibling is desired | **`stop-at: pr-open` required** for the implement leaf | Orchestrator spawns sibling review-monitor + lease. Cursor / Claude Code `drive-to: merge-ready` alternative is blocking `pr:watch` in the leaf process. |
| **Grok through-merge (#4529 / #4130)** | **`stop-at: pr-open` required** for the implement leaf | Named partner is Approach 1 sibling or parent-retained. Phase 6 / `swarm:finalize-cohort` owns squash-merge and leftover. ⊗ Harvest a Grok `drive-to: merge-ready` continuation as that partner. Blocking `pr:watch` in the Grok implement leaf is not the through-merge envelope. |
...
! Recovery: parent/monitor ground-truths once, then backgrounds **exactly one** continuation owner scoped `drive-to: merge-ready` on the same worktree/PR **or** one review-cycle babysit owner with sticky `<!-- deft:review-owner -->` lease. **Grok through-merge (#4529):** recovery MUST be Approach 1 sibling or parent-retained — ⊗ harvest a Grok `drive-to: merge-ready` continuation as that partner. ⊗ Dual lease / parallel babysit (#3044). ⊗ Cursor global babysit freestyle (#2261).
