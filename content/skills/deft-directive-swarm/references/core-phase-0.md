...
! **Identity-only first-scope skip (#4390):** if a proposed scope was synthesized from PROJECT-DEFINITION Overview, directory name, or init seed without a captured product or slice description, do not promote it. Identity-only setup must not emit that file; if one is present, treat it as out of scope for this bridge and ask the operator to run Starting-new or capture a slice description. ⊗ Treat an identity-only filler proposed xBRIEF as swarm-ready work.
