...
! **Worker liveness gate (#2824):** For every in-flight `drive-to: merge` or `drive-to: merge-ready` Cursor `Task` leaf, the monitor MUST run `task verify:subagent-alive -- --require-agent <agent-id> [--scratch-dir <worktree>/.deft-scratch/subagent-status]` at each poll iteration BEFORE treating the worker as healthy. Exit `1` prints `REDISPATCH_OK` — authorize takeover/re-dispatch even when the host reports the Task as "still running" (Cursor false-alive). Exit `2` is config error. Do NOT infer liveness from host lifecycle alone on the Cursor path.

⊗ Treat host-reported "running" as sufficient proof of worker health when `task verify:subagent-alive` exits non-zero for a registered in-flight `drive-to: merge*` worker (#2824 / cohort-2804-2814 recurrence).
...
! **Worker liveness gate (#2824):** Before claiming any in-flight `drive-to: merge*` implementer is healthy during Phase 5 review polling, run `task verify:subagent-alive` with `--require-agent` for each registered leaf. On exit `1` (`REDISPATCH_OK`), re-dispatch a takeover worker — do not wait on Cursor false-alive "still running" state. Pair with `task agent:monitor` raw sweeps from Phase 4 when scanning multiple worktrees.
