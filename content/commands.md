...
- Explicit probe: `deft scm:status` (alias `scm:readiness`) -- exit `0` ready / `1` not ready / `2` config; flags `--json`, `--deep` / `--shallow`, `--repo OWNER/REPO`, `--expected-login`, `--expected-app-slug`, `--expected-installation-id`. Deep validation derives the target repo and compares an expected worker principal when one is supplied (#3665).
