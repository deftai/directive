...
- 	ask scope:bind-clause -- <active-brief> --clause N[,M] --path <file_scope-member> -- explicit bind of selected pathless derived clauses onto an approved ile_scope matchAny path before stamp (#4986). Empty ile_scope is a silent no-op. Does not lift a path from issue or comment text (#3835).
- 	ask scope:stamp-evidence -- <brief> -- evidence-only writer: records kind:test via stampMatchAnyFileEvidence onto clauses that already have a non-blank in-scope rtifact_path (#4840 / #4986). No --pr / --merge-commit / disposition argv.
