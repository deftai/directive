...
- **Cursor Task parallel pre-push review (#4321):** `bugbot`, `security-review`, and unmarked `generalPurpose` stay implement-class. On Cursor Task, read-only parallel review uses `subagent_type` explore or continues in the parent. Do not use session assist, Grok-only `plan`, or cloud as the pre-push hatch. Bind later parallel review to the intended tree and SHA. `/preflight-loop` is not in this repository.
