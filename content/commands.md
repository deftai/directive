...
- **Cursor Task parallel pre-push review (#4321):** No dest skip for `bugbot`, `security-review`, or unmarked `generalPurpose`. Prompt-path dest and inherited parent cwd are not spawn dest. Implement dest stays fail-closed (#4066). On Cursor Task, read-only parallel review uses `subagent_type` explore or continues in the parent. Do not recut #2308 (host Bugbot / security-review stay advisory-only). Session assist is not that recovery. Cloud review stays on #4294 and is not a pre-push local hatch. Codec / Task dest-key measurement stays on #4295 and #4279 item 0. Any later parallel review path must bind results to the intended tree and SHA; unique dest-lock remains for write-capable children. `/preflight-loop` is not in this repository; consumer-command success is the Directive predicate plus a consumer note. Dest-missing deny copy for Cursor Task does not advertise Grok-only `plan`; explore is a read-only spawn, not only research.
