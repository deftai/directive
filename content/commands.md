...
- ! **Requirements posture (#4444):** declare via trusted `deft session:start --posture=requirements` (sets env for PreToolUse). Occupancy runs before any allow. Path class uses `evaluateRuntimeAuthorityPath` (default allow: docs/**, specs/**, README.md, xbrief/proposed/**; deny instruction surface). Out-of-class writes refuse with a mutation upgrade path. Do not reuse `write-assist-scratch-ready`. Payload posture fields are not a trusted producer.
