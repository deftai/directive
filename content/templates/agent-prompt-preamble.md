...
- `dispatch_provider`: the runtime primitive that launched this worker -- e.g. `spawn_subagent`, `start_agent`, `sessions_spawn` (OpenClaw), `cursor-composer`, `cursor-cloud-agent`, or a future adapter id. Names the harness surface, not the model.
...
- `selected_backend`: the stable backend id from `plan.policy.swarmSubagentBackend` / `task policy:subagent-backends` (e.g. `composer`, `grok-build`, `cursor-cloud`, `openclaw`) | null -- which catalogued coding backend the operator selected for this role.
