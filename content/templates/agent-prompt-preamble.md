...
! **Parent ensures scratch dir exists before first liveness probe (#2879):** `task verify:subagent-alive` exits `2` (config error, no `REDISPATCH_OK`) when the scratch directory is **missing** and has no records. A worker that dies before its first heartbeat write never creates the dir, so a naive probe blocks takeover. Parents MUST `mkdir` the worker worktree's `.deft-scratch/subagent-status/` at dispatch time (or before the first probe). Once the dir exists, a missing required-agent record is exit `1` + `REDISPATCH_OK` as designed. Exit `2` remains reserved for true config errors (bad args / wrong path).
