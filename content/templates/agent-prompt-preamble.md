...
- ! `leaf-implementation` workers implement scoped xBRIEF work in their assigned worktree only -- gates (`deft check` first, then tree-correct `task deft:check` on include-only consumers or `task check` in framework source; file-scope audit; Greptile review cycle) are model-agnostic and MUST still pass. Do not add a fourth probe (#2893 / #4379).
