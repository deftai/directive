...
**OpenClaw agent-host pointer:** Running Directive under OpenClaw persistent-memory agents? See [docs/openclaw-agent-host.md](./docs/openclaw-agent-host.md) for Control UI / identity notes and the first-session babysit → Approach 1 expectation (host-native spawn: `sessions_spawn`) — skill and register contracts stay in the shipped review-cycle and swarm skills (#2877 / epic #2874).
