...
Mint writes one record and one preimage as a fail-closed pair: a dest-write failure restores the previous pair or leaves neither dest. `intentDigest` is a checksum of `.deft/approved-scope/<plan-id>.intent.json` (`intent-extract-v1`).
