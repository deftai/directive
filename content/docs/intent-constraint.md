...
6. Human-authored mint for `value`, `unit`, and `rejectionScope` via `task scope:record-intent-constraint -- <xbrief> --actor <you> --confirm`. Commit `.deft/intent-constraint/<plan-id>.json` on the merge base before the implementation PR. Authority read is `git show <merge-base>` only. Same-PR rewrite fails. Multiple merge-base mints require `--plan-id` or `DEFT_ACTIVE_SCOPE`. Each mint row covers at most one new numeric-const of that value and at most one new throw/reject/abort site.
