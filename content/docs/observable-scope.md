...
3. Collection UX (#5010): attended harness → ask the human in the parent chat; on yes land `.deft/observable-scope/<plan-id>.json` on the merge base **before** the UI-change PR with `mintedVia: in-harness-ask`. Unattended / C1 → restore baseline markup or park — leave-harness TTY `scope:record-observable-scope` is not the primary path (legacy repair only). Same-PR rewrite fails. `--actor` is display-only. The fail-closed missing-mint site is verify merge-base, not a same-run prompt. Parent-agent auto-approve is out of scope.
...
One remediation: restore the baseline markup structure, or ask in the parent chat to land the merge-base record; leave-harness TTY `scope:record-observable-scope` is legacy repair only.
