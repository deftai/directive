...
def test_greenfield_pd_export_tolerated_without_spec_vbrief(tmp_path: Path):
    """Greenfield PROJECT-DEFINITION export is not a forbidden legacy dual-write."""
    mod = _load_module()
    vbrief = tmp_path / "vbrief"
    vbrief.mkdir()
    for dname in ("proposed", "pending", "active", "completed", "cancelled"):
    ...
    (vbrief / "proposed" / "2026-06-28-foo.vbrief.json").write_text("{}", encoding="utf-8")

    errors = mod.validate_strategy_output(tmp_path)
    assert not any("specification.vbrief.json" in e for e in errors), errors


def test_migration_fidelity_fails_when_premigrate_narratives_unmigrated(tmp_path: Path):
    mod = _load_module()
    vbrief = tmp_path / "vbrief"
    vbrief.mkdir()
    (vbrief / "PROJECT-DEFINITION.vbrief.json").write_text(
        '{"plan":{"narratives":{"Overview":"ok"}}}', encoding="utf-8"
    )
    (vbrief / "specification.premigrate.vbrief.json").write_text(
        '{"plan":{"narratives":{"ProblemStatement":"lost","Goals":"lost"}}}',
        encoding="utf-8",
    )

    errors = mod.validate_strategy_output(tmp_path)
    assert any("#2005" in e and "problemstatement" in e.lower() for e in errors)


def test_migration_fidelity_passes_when_narratives_migrated(tmp_path: Path):
    mod = _load_module()
    vbrief = tmp_path / "vbrief"
    vbrief.mkdir()
    (vbrief / "PROJECT-DEFINITION.vbrief.json").write_text(
        '{"plan":{"narratives":{"Overview":"ok","ProblemStatement":"here","Goals":"here"}}}',
        encoding="utf-8",
    )
    (vbrief / "specification.premigrate.vbrief.json").write_text(
    ...
    errors = mod.validate_strategy_output(tmp_path)
    assert not any("#2005" in e for e in errors)
