...
- **fix(migrate): self-validate output; slug-safe IDs; golden fixture** (#498, #506, #495): Added scripts/_vbrief_validation.py with a single slugify_id() sanitiser used for BOTH filename slug components AND every in-JSON identifier (plan.items[*].id, plan.id, scope-registry ids) emitted by the migrator -- lowercase ASCII letters, digits, and hyphens only; 80-char ceiling; stable 6-char sha1 hex suffix on collision. Conforms to the schema-locked ID regex ^[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)*$ (#506 Shared Conventions). Exports alidate_migration_output(), isolate_invalid_output(), inalize_migration(), and slug_fallback_id() helpers. scripts/migrate_vbrief.py now runs the same rule set as scripts/vbrief_validate.py on every emitted file as its terminal step (anchor # --- validation (Agent D, #498) ---) and hard-blocks on schema-invalid output per #506 D8: prints per-file diagnostics to stderr, moves the partial brief/ tree to brief.invalid/ (numeric suffix on collision) so Agent C's .premigrate.* backups (#497) remain available for 	ask migrate:vbrief -- --rollback recovery, and returns non-zero. _build_project_definition now always emits a synthesized 	ech stack narrative alongside Overview (mirrors the #417 pattern) so minimal fixtures pass D3 validation out of the box. Added synthetic golden fixture under 	ests/fixtures/pre_cutover_customized/ (SPECIFICATION.md with [done] markers + FR-N traces + Depends on: lines, PROJECT.md with custom tech stack, ROADMAP.md with phase/tier/completed sections and titles containing slug-unsafe characters <, >, /, @, spaces) with a matching pre_cutover_customized.expected/ tree; TestGoldenFixture in 	ests/cli/test_migrate_vbrief.py asserts byte-for-byte equality after canonical JSON re-serialisation and runs scripts/vbrief_validate.py as a subprocess on the output to tie the CI 	ask check gate to the fixture. 35 unit tests in new 	ests/cli/test_vbrief_validation.py; 10 integration tests appended to 	ests/cli/test_migrate_vbrief.py (TestSlugSafeIds, TestMigrationSelfValidation, TestGoldenFixture).
