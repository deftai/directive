...
### Fixed
- **ci(release): fall back from Blacksmith ubuntu runners to `ubuntu-latest` on the `build` and `release` jobs** (v0.20.1 incident follow-up): `.github/workflows/release.yml` previously used `runs-on: blacksmith-4vcpu-ubuntu-2404` for the cross-compile build matrix and the release-publish step. On the v0.20.1 tag push (2026-04-23) Blacksmith's ubuntu pool starved the `darwin/arm64` matrix cell for 50+ minutes while every other cell picked up a runner in <60 seconds, blocking `universal-macos`, `smoke-test`, and `release` downstream. Switched both jobs to GitHub-hosted `ubuntu-latest` which has much higher reserved capacity. The `universal-macos` job was already `macos-latest` (fallback from the earlier Blacksmith macOS networking incident); `smoke-test` continues to use `ubuntu-latest` / `windows-latest` / `macos-latest` / `ubuntu-24.04-arm` as before. Trade-off is slower per-minute execution than Blacksmith in exchange for reliability. Inline comment on the job documents the reason so future maintainers understand the config choice. See `meta/lessons.md` "Runner Pool Reliability" entry for the general pattern.
...
## [0.20.1] - 2026-04-23 [YANKED]

Tag `v0.20.1` was pushed on 2026-04-23 at 16:18 UTC but the release workflow stalled on third-party runner capacity starvation (Blacksmith `blacksmith-4vcpu-ubuntu-2404` pool). The `darwin/arm64` matrix cell of the cross-compile `build` job remained queued for 54 minutes before a Blacksmith runner was finally assigned, while every other matrix cell picked up a runner in <60 seconds. By t+54m the operator had judged the queue-time SLA unsalvageable for a release cut and submitted a cancel at 17:12 UTC; the darwin/arm64 job completed successfully just 9 seconds before the cancel propagated to the downstream `universal-macos`, `smoke-test`, and `release` jobs, which were therefore never given a chance to run. **No installer binaries were published for v0.20.1 and no GitHub Release was created** -- the tag is a zombie anchored at commit `493050b` with no release attached.

The docs-only fix originally intended for v0.20.1 (post-migration render step + `ROADMAP.md` deprecation-scope correction, see commit `493050b`) ships in v0.20.2 alongside the `ci(release):` runner-config change that prevents the same starvation from affecting future releases. Users upgrading from v0.20.0 should target v0.20.2 directly; do not attempt to pin to v0.20.1.
