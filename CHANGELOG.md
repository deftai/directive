...
### Added
- **feat(scripts): add task issue:ingest for GitHub issue -> vBRIEF ingestion** (#454): Created `scripts/issue_ingest.py` with single-issue mode (`task issue:ingest -- <N> [--status proposed|pending|active]`) and bulk mode (`--all [--label L] [--status S] [--dry-run]`). Fetches via `gh api` / `gh issue list`, deduplicates against existing `references[type=github-issue]` entries using `scripts/reconcile_issues.py::scan_vbrief_dir`, and writes a scope vBRIEF with origin provenance to `vbrief/<status>/YYYY-MM-DD-<N>-<slug>.vbrief.json`. Exit codes: 0 success, 1 duplicate in single-issue mode, 2 on gh/API error. Extracted the shared `slugify` and `create_scope_vbrief` helpers from `scripts/migrate_vbrief.py` into a new `scripts/_vbrief_build.py` module so both scripts import the same implementation (migrate_vbrief re-exports the legacy underscore-prefixed aliases for backward compatibility with existing tests). Wired `task issue:ingest` via `tasks/issue.yml`. Added `tests/cli/test_issue_ingest.py` covering single-issue happy path, dedup hit (+ CLI exit 1), bulk summary counts, label filter, dry-run (single + bulk), gh API error (CLI exit 2), and --help subprocess. Documented in `vbrief/vbrief.md` Quick Reference, `README.md` (new "Ingesting GitHub issues" subsection and two Document Generation table rows), and `UPGRADING.md` v0.20 expectations.
- **docs(readme): remove v0.20.0 RC testing safety banner** (#429): Removed the v0.20.0 release-candidate banner from `README.md` that directed testers to run `task migrate:vbrief` against a fork or copy and noted "we are still refining" RC language. Moved a distilled, version-agnostic "test on a fork or copy first" paragraph to `UPGRADING.md` under a new "Upgrade safety" subsection in the v0.20 section so upgrade-safety advice is preserved without RC-specific wording. README no longer contains any RC-specific or in-progress refining language ahead of the v0.20.0 final tag.
- **docs(glossary): add deft/glossary.md canonical v0.20 vocabulary** (#457): Created `glossary.md` at the repo root with the RFC2119 legend and exactly 13 required v0.20 vocabulary terms (Scope vBRIEF, Lifecycle folder, Plan-level narrative, Item-level narrative, Filename stem, Cross-scope dependency, Exit Commands, Origin provenance, Canonical narrative key, Preparatory strategy, Spec-generating strategy, Rendered export, Source of truth). Each entry uses the bold-term + 1-2 sentence definition + authoritative cross-link pattern and the file stays under 150 lines. Added cross-references to `glossary.md` from `vbrief/vbrief.md` See-also banner, `README.md` (new "Glossary" section alongside Skills), and `UPGRADING.md` v0.20 References list. New content tests in `tests/content/test_glossary.py` assert file existence, line count < 150, RFC2119 legend, all 13 term labels present with at least one cross-link each, and cross-reference coverage across the three target docs.
