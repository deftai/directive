...
- **feat(docs): pre-cutover migration discoverability from project root + AGENTS.md** (#500, #506 D6): Added a ## Migrating from pre-v0.20 section to main.md documenting the canonical 	ask migrate:vbrief invocation, the 	ask -t ./deft/Taskfile.yml migrate:vbrief fallback when the primary task is not resolvable, the recommended includes: deft: deft/Taskfile.yml pattern for publishing deft tasks from the project root, what the migrator produces (RECONCILIATION.md, LEGACY-REPORT.md), and --dry-run / --rollback / --strict / --force safety flags; added an explicit Pre-Cutover Check branch to 	emplates/agents-entry.md (and mirrored the same content in the Go installer constant gentsMDEntry in cmd/deft-install/setup.go) so agents following the consumer AGENTS.md literally are routed to migration BEFORE Phase 1 / Phase 2 / Returning-Sessions; rewrote QUICK-START.md Case H/I to reference the fallback invocation and cross-link the new main.md migration section; extended skills/deft-directive-setup/SKILL.md Pre-Cutover Detection Guard with an Environment Preflight subsection (task resolvability grep, uv on PATH check, deft/scripts/migrate_vbrief.py presence check) that reports results BEFORE the yes/no run prompt, and an explicit actionable fallback command for when 	ask migrate:vbrief is not resolvable. Install-step Taskfile mutation (#500 option ii) is explicitly skipped per #506 D6 -- the include pattern is the supported publish mechanism. New content tests in 	ests/content/test_500_discoverability.py.
