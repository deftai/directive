...
| [deft-directive-triage](./content/skills/deft-directive-triage/SKILL.md) | Withdrawn (#4070). Classify playbook is off. Work selection is plan-sequence:current then a read-only triage:queue listing. Replacement sieve is #4071. | `triage hygiene`, `work the cache`, `what's next`, `queue`, `build a cohort` |
| [deft-directive-portfolio-priority](./content/skills/deft-directive-portfolio-priority/SKILL.md) | Pre-promotion portfolio prioritization: cluster competing open RFCs/issues from the local cache, emit a propose-not-apply priority brief (conflict matrix, shortlist, park) with epistemic gates, then hand off for operator dispose (#3198 / #3201). Worked example: #3200 pilot brief. | `portfolio priority`, `priority brief`, `competing RFCs`, `cluster open issues`, `pre-promotion portfolio`, `portfolio prioritization` |
...
| [deft-directive-probe](./content/skills/deft-directive-probe/SKILL.md) | Adversarial one-question-per-turn plan stress-testing before any xBRIEF or plan artifacts are written. Mechanical CLI: `deft probe-session`. | `run probe`, `/deft:run:probe`, `probe` |
...
| [deft-directive-xbrief](./content/skills/deft-directive-xbrief/SKILL.md) | On-demand **create/verify** dense xBRIEF artifacts (`json`/`md`/`both`) at `--out` with pack-on-use templates. **Not** scope lifecycle (`scope:*`). | `xbrief create`, `xbrief verify`, `create xbrief`, `verify xbrief`, `compress into xbrief`, `xbrief:create`, `xbrief:verify`, `xbrief:adopt-stored-plan-id` |
...
| [deft-directive-design-critique](./content/skills/deft-directive-design-critique/SKILL.md) | Thin router into the design-critique contract: five pointer stops; operator dispatches from the brief template. | `design critique`, `design-critique`, `critique panel`, `mechanism-shaped triage`, `arc`, `run an arc` |
| [deft-directive-issue-eval](./content/skills/deft-directive-issue-eval/SKILL.md) | Thin router into the issue-eval contract: isolated origin/master validity, parent WIP census, named sink. Operator runs `task triage:evaluate`. | `issue-eval`, `issue eval`, `triage:evaluate`, `evaluate issues` |
