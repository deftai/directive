...
| [deft-directive-portfolio-prioritize](./content/skills/deft-directive-portfolio-prioritize/SKILL.md) | Pre-promotion portfolio prioritization from the local github-issue cache: cluster competing RFCs/issues, name conflicts/supersession, emit a priority brief (shortlist + park list) under propose-not-apply. Does **not** replace `triage:queue` or write SCM labels (#3198 / #886). | `portfolio prioritization`, `priority brief`, `portfolio pass`, `prioritize portfolio`, `shortlist and park`, `RFC portfolio`, `park list` |
