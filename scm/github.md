...
- ! For BOM-safe file writes after agent reads, use `[System.IO.File]::WriteAllText($path, $content, [System.Text.Encoding]::UTF8)` or `Out-File -Encoding utf8NoBOM` -- never use `Set-Content` or `Out-File` without explicit encoding in PS 5.1, as these inject a BOM
...
- ! Never paste multi-line PowerShell string literals (here-strings `@" ... "@`) directly into the Warp agent input box -- Warp splits multi-line input across separate command blocks, causing syntax errors or silent truncation. Always write multi-line PS content to a temp file first (e.g. `Set-Content $tmpFile $content`), then use the temp file path in subsequent commands
