...
- ! For BOM-safe file writes after agent reads, use `[System.IO.File]::WriteAllText($path, $content, [System.Text.UTF8Encoding]::new($false))` or `Out-File -Encoding utf8NoBOM` -- never use `Set-Content` or `Out-File` without explicit encoding in PS 5.1, as these inject a BOM
