    ...
    if spec_vbrief_path.exists():
        ...
        if isinstance(spec_vbrief, dict):
            ...
            if isinstance(envelope, dict) and envelope.get(
                ...
                actions.append(
                    "BUMP  specification.vbrief.json vBRIEFInfo.version "
                    f"{prior_version!r} -> {EMITTED_VBRIEF_VERSION!r} (#571)"
                )
                # Persist the bump immediately so even a no-narrative-
                # ingest migration lands v0.6 on disk. Subsequent ingest
                # writes may re-serialize the same (already-bumped)
                # in-memory copy; that is harmless because the envelope
