...
- ``documents`` mode renders each entry that carries a body into its own
  per-entry path. The skills pack uses it: the ONE designated proof skill's
  full body -> its ``skills/<name>/SKILL.md`` (reconstructed YAML frontmatter +
  provenance banner + body). Metadata-only entries (body null) are skipped.
...
def _targets_for_pack(
    ...
    for entry in pack.get(cfg["items_field"], []):
        ...
        targets.append((out_path, render_skill_document(entry, cfg)))
    return targets
